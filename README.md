...
WorkBuddy AI Skill - 通用图片创作技能，基于 API Router (gpt-image-2)，支持文生图和图生图两种模式。
...
├── scripts/
│   └── generate.py       # 图片生成脚本（API Router 调用）
├── .gitignore
└── README.md
```
...
- 需要有效的 API Router Key
- 模型：`gpt-image-2`
- API 端点：`https://apirouter.myevia.io/v1`
- Python 3.9+（脚本仅依赖标准库）
